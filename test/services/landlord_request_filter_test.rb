require "test_helper"

class LandlordRequestFilterTest < ActiveSupport::TestCase
  setup do
    @landlord_user1 = User.create!(
      fullname: "Landlord One",
      tel: "0901111111",
      password: "Password123",
      password_confirmation: "Password123",
      role: "landlord",
      sex: "male",
      bday: 35.years.ago.to_date,
      address: "123 Landlord St",
      tel_verified_at: Time.current
    )
    @landlord1 = Landlord.find_or_create_by!(id: @landlord_user1.id)

    @landlord_user2 = User.create!(
      fullname: "Landlord Two",
      tel: "0902222222",
      password: "Password123",
      password_confirmation: "Password123",
      role: "landlord",
      sex: "male",
      bday: 40.years.ago.to_date,
      address: "456 Other St",
      tel_verified_at: Time.current
    )
    @landlord2 = Landlord.find_or_create_by!(id: @landlord_user2.id)

    @tenant_user = User.create!(
      fullname: "Tenant Le",
      tel: "0907654321",
      password: "Password123",
      password_confirmation: "Password123",
      role: "tenant",
      sex: "female",
      bday: 22.years.ago.to_date,
      address: "789 Tenant Rd",
      tel_verified_at: Time.current
    )
    @tenant = Tenant.find_or_create_by!(id: @tenant_user.id)

    @house_a = House.create!(
      landlord: @landlord1,
      name: "House A",
      mode: :room,
      address_l1: "123 Main St",
      address_l2: "Ward 1",
      address_l3: "District 1",
      floors_count: 1,
      inv_creation_date: 1
    )

    @house_b = House.create!(
      landlord: @landlord1,
      name: "House B",
      mode: :room,
      address_l1: "456 Side St",
      address_l2: "Ward 2",
      address_l3: "District 2",
      floors_count: 1,
      inv_creation_date: 1
    )

    @house_other = House.create!(
      landlord: @landlord2,
      name: "Other House",
      mode: :room,
      address_l1: "999 Other St",
      address_l2: "Ward 9",
      address_l3: "District 9",
      floors_count: 1,
      inv_creation_date: 1
    )

    # Request 1 in House A, pending, now
    @v_req1 = create_vehicle_request("59A-11111")
    @req1 = Request.create!(
      tenant: @tenant,
      house: @house_a,
      requestable: @v_req1,
      status: :pending,
      created_at: Time.current
    )

    # Request 2 in House B, approved, 1 month ago
    @v_req2 = create_vehicle_request("59B-22222")
    @req2 = Request.create!(
      tenant: @tenant,
      house: @house_b,
      requestable: @v_req2,
      status: :approved,
      created_at: 1.month.ago
    )

    # Request 3 in Other House belonging to Landlord 2
    @v_req3 = create_vehicle_request("59C-33333")
    @req3 = Request.create!(
      tenant: @tenant,
      house: @house_other,
      requestable: @v_req3,
      status: :pending,
      created_at: Time.current
    )
  end

  def create_vehicle_request(plate)
    vr = VehicleRequest.new(
      license_plate: plate,
      vehicle_type: :motorbike,
      consent_given_at: Time.current
    )
    vr.registration_card_image.attach(
      io: File.open(Rails.root.join("test/fixtures/files/normal.png")),
      filename: "reg.png",
      content_type: "image/png"
    )
    vr.vehicle_photo.attach(
      io: File.open(Rails.root.join("test/fixtures/files/normal.png")),
      filename: "photo.png",
      content_type: "image/png"
    )
    vr.save!
    vr
  end

  test "returns all requests across all houses of the landlord by default" do
    results = LandlordRequestFilter.call(landlord: @landlord1, params: {})

    assert_includes results, @req1
    assert_includes results, @req2
    assert_not_includes results, @req3
  end

  test "filters by specific house_id" do
    results = LandlordRequestFilter.call(landlord: @landlord1, params: { house_id: @house_a.id.to_s })

    assert_includes results, @req1
    assert_not_includes results, @req2
    assert_not_includes results, @req3
  end

  test "filters by status" do
    results = LandlordRequestFilter.call(landlord: @landlord1, params: { status: "approved" })

    assert_includes results, @req2
    assert_not_includes results, @req1
  end

  test "filters by month and year" do
    past_date = 1.month.ago
    results = LandlordRequestFilter.call(
      landlord: @landlord1,
      params: { month: past_date.month.to_s, year: past_date.year.to_s }
    )

    assert_includes results, @req2
    assert_not_includes results, @req1
  end

  test "filters by request_type" do
    results = LandlordRequestFilter.call(landlord: @landlord1, params: { request_type: "VehicleRequest" })
    assert_equal 2, results.size

    results_none = LandlordRequestFilter.call(landlord: @landlord1, params: { request_type: "RepairRequest" })
    assert_equal 0, results_none.size
  end
end
